...
- 본인이 진행한 프로젝트의 `layout.tsx` 하위 페이지에는 총 4개의 맵 객체가 필요했기 때문에, 총 4개의 context를 생성했다.
...
- 마찬가지로, 본인이 진행한 프로젝트의 `layout.tsx` 하위 페이지에는 총 4개의 map 객체가 필요했기 때문에, 총 4개의 context에 해당하는 `Component`를 생성했다.
...
- 지도의 일반보기, 위성보기 기능을 사용하기 위하여 각 `TileLayer`를 `return`하는 함수를 만들 것이다.
	- `types/common/map/Layer.ts`라는 이름으로 만들었다. (만드는 건 자유로이 만들면 된다.)
	- 각 이름은 `base-vworld`, `base-vworld-Satellite`이다.

```ts
import TileLayer from "ol/layer/Tile";
import { XYZ } from "ol/source";

export const getVWorldLayer = () =>
  new TileLayer({
    className: "base-vworld",
    source: new XYZ({
      // 개발용
      url: `http://api.vworld.kr/req/wmts/1.0.0/개발용 API KEY/Base/{z}/{y}/{x}.png`,

      // 배포용
      // url: `http://api.vworld.kr/req/wmts/1.0.0/배포용 API KEY/Base/{z}/{y}/{x}.png`,
    }),
...
// vworld Satellite
export const getVWorldSatelliteLayer = () =>
  new TileLayer({
    className: "base-vworld-Satellite",
    source: new XYZ({
      // 개발용
      url: `https://api.vworld.kr/req/wmts/1.0.0/개발용 API KEY/Satellite/{z}/{y}/{x}.jpeg`,

      // 배포용
      // url: `https://api.vworld.kr/req/wmts/1.0.0/배포용 API KEY/Satellite/{z}/{y}/{x}.jpeg`,
    }),
