...
- 대시보드 개요 페이지의 데이터를 가져오기 위해, Learning Course에서는 다음과 같은 코드를 제공하고 있다. 
	...
	- 해당 파일은 비동기(`async`) 컴포넌트이며, `await`문을 사용하여 데이터를 가져올 수 있다.
```tsx
...
- 다음으로, `/app/lib/data.ts`에 정의된 `fetchRevenue()`를 해당 파일에서 import하여, revenue 데이터를 불러와야 한다.
	- 아래와 같이, `await`문을 사용하여, 데이터를 fetch할 수 있다.
	- 결과 확인을 위해서는, 주석처리된 `<RevenueChart>` 컴포넌트의 주석을 해제해야 한다.
...
- 다음으로는, `<RevenueChart>`컴포넌트 (`/app/ui/dashboard/revenue-chart.tsx`) 코드에서 차트를 그리는 부분을 주석 해제 해야한다.
	- 그 결과 코드는 아래와 같다.
```tsx
...
- 여기까지 잘 따라왔다면, 아래와 같은 화면을 볼 수 있을 것이다.
![[Pasted image 20240302112654.png]]
