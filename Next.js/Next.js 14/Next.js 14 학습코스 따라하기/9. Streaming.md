...
- **이미지 1**
![[Pasted image 20240318103411.png]]

- **이미지 2**
![[Pasted image 20240318103451.png]]
...
- **결과**
![[Pasted image 20240318173844.png]]
...
- 전체 페이지를 `loading.tsx`에서 했던 것처럼 스트리밍할 수 있다.그러나 컴포넌트 중 하나가 데이터를 느리게 가져오는 경우 로딩 시간이 길어질 수 있다. 이런 경우, 사용자 경험에서 분명 모든 컴포넌트를 개별적으로 스트리밍할 수 있습니다... 그러나 화면에 준비가 되면 UI가 화면에 갑자기 나타날 수 있습니다. 또한 페이지 섹션을 스트리밍하여 지연 효과를 만들 수 있습니다. 그러나 래퍼 컴포넌트를 만들어야 합니다. Suspense 경계를 어디에 놓을지는 애플리케이션에 따라 다를 것입니다. 일반적으로 데이터를 필요로 하는 컴포넌트로 데이터 가져오기를 이동한 다음 해당 컴포넌트를 Suspense로 래핑하는 것이 좋은 실천 방법입니다. 그러나 애플리케이션이 필요로 하는 것이라면 섹션 또는 전체 페이지를 스트리밍하는 것에는 문제가 없습니다.

Suspense를 실험하고 어떤 것이 최선인지 확인하는 것을 두려워하지 마세요. 이는 사용자 경험을 더욱 즐겁게 만들어 줄 수 있는 강력한 API입니다.