...
- 지금까지는 **전체 페이지**를 스트리밍하는 방법에 대해 알아보았다. 
	...
	- **React Suspense**는원하는 특정  **Component**에 한정하여 더욱 세분화된 스트리밍을 진행할 수 있도록 한다.

- Suspense를 사용하면, 일부 조건(데이터 로드)을 충족할 때 까지, 애플리케이션의 렌더링 부분을 연기할 수 있다. 즉, Suspense에서 동적 구성요소를 