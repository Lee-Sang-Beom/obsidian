...
- 다음으로, `/app/lib/action.ts` 파일로 이동해서, Form 검증 규칙을 정의한 FormSchema를 아래와 같이 변경해주자.
	1. **customerId**: Zod는 기본적으로 'customerId'가 문자열이어야 하므로, 고객을 선택하지 않으면 오류를 발생시킨다. 하지만 여기서는 추가로 '고객을 선택하세요'라는 메시지를 지정하여 사용자에게 친절한 안내를 제공한다.
	2. **amount**: 'amount' 필드를 문자열에서 숫자로 강제 변환(`.coerce()`)한다.  만약 빈 문자열을 입력하면 자동으로 0으로 변환되는데, `.gt(0)` 메서드를 사용해 항상 0보다 큰 숫자만 허용하도록 했다. 따라서, 0 이하의 값을 입력하면 '0보다 큰 금액을 입력하세요'라는 메시지가 출력된다.
	3. **status**: Zod는 'status' 필드가 'pending' 또는 'paid' 값 중 하나여야 한다고 강제한다. 값이 비어 있거나 유효하지 않으면 '송장 상태를 선택하세요'라는 사용자 친화적인 오류 메시지를 추가한다.
