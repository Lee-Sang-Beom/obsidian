...
- 일단, 로그아웃을 구현함에 앞서, 로그아웃을 할 수 있는 로그인의 상태인지 먼저 확인해야 한다.
	- 이를 위해, Next.js의 `layout.tsx`에서, `getServerSession()`메소드로 `session`이 존재하는지를 확인했다.
	- `session`이 존재하면, 로그인 상태이다.

- 언젠가, 팀원이 해당 프로젝트의 로그인 관리부분을 `copy/paste`하다가 작동이 안된다고 하여, 질문을 하러 온 적이 있었다. `getServerSession()`작동이 안된다는 것이었다.
	- 알아보니, 공식문서에서는 `next-auth/next`를 사용한다면, 다르게 `import`해야 하는 간단한 문제였다. 잘 알아보고 사용할 수 있도록 하자.
```tsx
// ...
