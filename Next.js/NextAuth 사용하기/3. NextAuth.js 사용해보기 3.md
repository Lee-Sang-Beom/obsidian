...
- Next.js의 미들웨어 파일은`(middleware.ts|.js)`, 요청이 완료되기 전에 서버 측에서 실행되는 함수이다.
	- 쉽게 말해, 클라이언트의 요청이 서버에 도착하기 전에 실행되어, 서버가 요청을 처리하기 전에 특정 작업을 할 수 있도록 돕는 파일이다.
	- 	- 수신된 요청을 기반으로 응답을 수정하여 리디렉션하거나 요청 또는 응답 헤더를 수정하거나 직접 응답하는 등의 작업을 수행할 수 있습니다.
