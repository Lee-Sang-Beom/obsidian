...
- Next.js에서 사용하는 SSR은 전통적인 SSR과는 거리가 있다. SSR과 CSR의 장점을 최대한 취하기 위해 절충점을 찾은 형태라 볼 수 있다.
	- Next.js는 초기 로딩속도가 느리다는 CSR 단점을 보완하기 위해서, 서버에서 HTML파일을 SSR방식으로 빠르게 받아오고, 병렬적으로 자바스크립트 번들(Javascript Bundle)을 함께 다운로드하여 미리 받아온 html과 병합하는 Hydration 과정을 거치게 된다.

- 이러한 Next.js의 렌더링 방식이 RSC와 함께  관련 좀 더 효율성 있는 렌더링을 진행할 수 있도록 구성되는데, 그 내용은 아래와 같다.
