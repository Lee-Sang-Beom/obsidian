...
- **React의 큰 특징 중 하나**는 Virtual DOM이다.
	...
	- `ReactDOM.render()`는 초기 렌더링에서만 사용되며, 이후 컴포넌트의 상태(state)나 속성(props) 변경으로 인해 업데이트가 필요할 때는 `Reconciliation` 과정을 통해 필요한 부분만 업데이트된다.

**※ 주의 : 서버 컴포넌트와 `render()`의 오해**
- 이어지는 아래 내용을 읽어보면 알겠지만, 서버 컴포넌트는 서버에서 HTML을 생성한 다음, 이 HTML을 클라이언트에게 전달하여 그대로 브라우저에 삽입하는 과정만 거칠 수 있도록 하기 때문에 `render()` 함수가 호출되지 않는다. 
	- **SSR**의 개념이 즉시 렌더링 가능한 HTML을 던져주는 과정에서 **"클라이언트가 렌더링을 해야 한다"** 고 생각할 수 있지만, 실제로는 **클라이언트가 HTML을 그대로 표시하고, 필요하면 상호작용을 활성화하는 역할**만 수행한다.

- 이런 경우, 클라이언트 측에서는 서버에서 이미 HTML을 완성해서 보냈기 때문에, 클라이언트는 별도로 `ReactDOM.render()`를 호출하여 새롭게 DOM을 생성할 필요가 없다.

> `hydrate()`
- 그럼, `reactDOM.hydrate()`는 뭘까?
	- `ReactDOM.hydrate()`는 **서버 사이드 렌더링(SSR)** 과 관련된 React 메소드로, 서버에서 생성된 HTML을 클라이언트 측에서 React 컴포넌트와 연결(재활용)하기 위해 사용된다.

 - **`ReactDOM.hydrate()`의 역할**은 아래와 같다.
...
| Feature      | `ReactDOM.render()`                   | `ReactDOM.hydrate()`                     |
| ------------ | ------------------------------------- | ---------------------------------------- |
| **사용 목적**    | 클라이언트 측에서 완전히 새로운 React 앱 렌더링         | 서버에서 전달된 HTML에 React 컴포넌트를 연결(Hydration) |
| **HTML 처리**  | 기존 HTML을 무시하고 새로 렌더링                  | 서버에서 전달된 HTML을 재활용하여 Virtual DOM과 연결     |
| **주된 사용 사례** | 클라이언트 렌더링(Client-Side Rendering, CSR) | 서버 사이드 렌더링(Server-Side Rendering, SSR)   |
| **성능**       | 모든 DOM을 새로 생성                         | 기존 DOM 재활용으로 더 빠르게 동작                    |
