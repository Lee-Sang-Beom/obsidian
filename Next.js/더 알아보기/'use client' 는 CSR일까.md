...
| 함수                   | 사용 시점 및 목적                                                             | 특징                                                                                          |
| -------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `ReactDOM.render()`  | 클라이언트 측 렌더링에서 처음 렌더링할 때 사용하며, React 컴포넌트 트리를 **HTML DOM**에 마운트하거나 업데이트 | React 애플리케이션을 브라우저의 실제 DOM에 연결할 때 호출된다. 주로 애플리케이션의 **최초 마운트** 또는 **업데이트** 시 호출              |
| `ReactDOM.hydrate()` | 서버 측 렌더링(SSR)에서 서버 렌더링 HTML을 React로 상호작용 가능하게 만들 때 사용.                 | 서버에서 렌더링된 HTML을 React와 연결, ReactDOMServer로 렌더링된 HTML에 이벤트 리스너(자바스크립트 코드)를 연결해주기 위해 사용       |
| `render()`           | 클래스형 컴포넌트 내에서 컴포넌트의 JSX를 반환하여 컴포넌트의 UI를 정의하는 역할 수행. (함수형 컴포넌트 x)       | JSX 반환 (상태 변경 시 재호출), 컴포넌트의 상태(state)가 변경되거나 **props**가 변경될 때 자동으로 호출되어 동적으로 UI를 변경하는 데에 사용 |

> `reactDOM.render()`
- `ReactDOM.render()`는 React 애플리케이션을 **HTML DOM에 렌더링**할 때 사용되는 메서드이다.
    - 이 메서드는 **최초 마운트** 또는 **업데이트** 시 호출된다.
    - `ReactDOM.render(<App />, document.getElementById('root'))`와 같이 사용되며, 실제 DOM에 **컴포넌트 트리를** 렌더링한다.
...
- `render()`는 **React 컴포넌트 내에서** 사용되는 **메서드**로, **컴포넌트의 JSX를 반환**한다.
	...
	- React는 이 메소드를 실행해서 반환됨 JSX를 바탕으로 **가상 DOM(Virtual DOM)** 을 만들어 실제 DOM과 비교하여 변경된 부분만 효율적으로 업데이트한다.
	- **`render()`는 **클래스형 컴포넌트**에서만 사용되며, 함수형 컴포넌트는 JSX를 반환하는 **단순한 함수**로 `render()` 메서드를 대체한다.
...
> 모범 답변:
- "`use client` 컴포넌트는 클라이언트에서 동작하지만, 서버에서 기본적인 HTML 구조를 렌더링한 후 클라이언트에서 Hydration 과정을 거쳐 완전한 UI로 동작합니다.
- 즉, 서버에서 일부 렌더링된 후 클라이언트에서 활성화되므로 **SSR이 가능하다고 볼 수 있습니다**. 하지만 서버 컴포넌트처럼 완전한 HTML을 생성하는 것은 아니며, Hydration이 필요하기 때문에 번들 크기가 커질 수 있습니다.  
	- `use client`를 선언한 컴포넌트 내부에서 **이벤트 핸들러가 필요한 부분만 Hydration을 위해 placeholder로 처리**되고, 나머지 정적인 부분은 SSR 방식으로 미리 렌더링되는 것입니다 (클라이언트 컴포넌트 전체가 placeholder되는 건 아님)
- 반면, 서버 컴포넌트는 Hydration 없이 순수 HTML을 제공하여 성능을 최적화할 수 있습니다."

- React 클라이언트 컴포넌트의 본질은 브라우저에서 실행되며, 렌더링과 로직이 클라이언트 측에서 동작하는 것이다.
	- 따라서 클라이언트 컴포넌트는 SSR 방식을 사용할 수 없다고 생각할 수 있지만, React의 `ReactDOMServer.renderToString()` 같은 API를 사용하면 클라이언트 컴포넌트의 초기 HTML을 서버에서 생성할 수 있다. 
	- 그리고, Next.js의 `use client` 컴포넌트는 SSR과 CSR을 혼합적으로 사용할 수 있게 설계되어 있다.
		- 서버에서 초기 HTML을 생성한 뒤, 클라이언트에서 React는 이 HTML을 기반으로 Virtual DOM과 동기화하고, 상태 및 이벤트 핸들링 등 동적 기능을 활성화하는 **Hydration 과정**을 거친다.
