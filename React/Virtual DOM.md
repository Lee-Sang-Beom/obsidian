...
- React는 **렌더링이 발생될 상황**에 놓이게 되면 새로운 화면에 들어갈 내용이 담긴 Virtual DOM을 생성한다.
	- Virtual DOM은 실제 DOM의 복사본으로, JavaScript 객체 형태로 메모리 내에 존재한다.
	- React는 항상 **렌더링 이전의 화면 구조**와 **렌더링 이후의 화면 구조**를 가진 **두 개의 Virtual DOM 객체**를 유지하고 있다.
		- 그리고 두 개의  Virtual DOM을 비교하여 **변경된 부분만 실제 DOM에 반영**한다.
