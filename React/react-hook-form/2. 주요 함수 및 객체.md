...
2. **데이터 검증**
    - 폼의 모든 필드에 대해 설정된 검증 규칙을 기반으로 데이터를 검증합니다. 예를 들어, `register` 함수로 각 필드에 대한 검증 규칙을 설정할 수 있습니다.

3. **폼 상태 관리**
    - `handleSubmit` 함수가 실행되는 동안 `isSubmitting` 상태가 `true`로 설정되어 폼이 제출 중임을 나타냅니다.
    - 제출이 완료되면 `isSubmitted` 상태가 업데이트됩니다.
