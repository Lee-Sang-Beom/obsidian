...
- 서식관리함 - 서식관리 - 서식 상세 데이터 불러오기 테이블 : ea_doc_main
	- 서식 자체에 대한 데이터를 불러오기 위함이며, 서식 자체에 대한 내용은 ea_doc_main에서 관리

- 기안문서작성 -  공용/단지서식 - 서식 상세 데이터 - 임시저장 및 상신을 위한 대상 테이블 :  ea_docapproval_main
	-  서식 자체에 대한 데이터에 사용자가 자기가 원하는 데이터를 덮어씌워 전자문서관리함의 문서로 상태를 변경하거나, 임시저장 상태로 변경했다면, 이는 ea_docapproval_main에서 관리된다.
...
6. 기안문서작성 -  공용/단지서식 - 서식 상세 데이터 - 임시저장
