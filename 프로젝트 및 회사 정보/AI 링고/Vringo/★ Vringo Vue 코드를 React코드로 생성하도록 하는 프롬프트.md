...
- 3단계
```  
좋아.위 vue코드는 이제 ssrc/pages/video/make-video/make-video-page.tsx와 연결해야해.
...
- vue 코드에서 VringoCategorySelect가 있었잖아. 그걸 react코드로 구현하기 전에 src/features/resource-management/resource/list/modals/edit/resource-edit-modal-presenter.tsx를 한번봐줘. 이걸 사용해서 구현할 수 있으면 이걸 재사용해도 돼. 만약 별도로 수정이 필요하다면 별도로 컴포넌트 만들어야 해

===============================================  

[구현 시 파일 규칙]  
- src/lib/category-parser.ts: vue의 categoryParser코드
- src/features/video/make-video/components: 해당 페이지 내에서 사용할 컴포넌트들을 이 디렉터리에 정의해줘. 
- src/features/video/make-video/modals: 해당 페이지 내에서 다이얼로그 컴포넌트가 필요할 때, 이 디렉터리에 정의해줘
- src/features/video/make-video/hooks/modals: 해당 페이지 내에서 다이얼로그 컴포넌트가 필요할 때, 다이얼로그 open 여부를 관리하는 훅을 여기 정의해줘
- src/features/video/make-video/hooks/queries/use-make-video-query-actions.ts: (구현완료) 해당 페이지 내에서 파일 등록을 위해 사용할 GET 요청 
- src/features/video/make-video/hooks/mutations/use-make-video-mutation-actions.ts: (구현완료) 해당 페이지 내에서 파일 등록을 위해 사용할 POST 요청 훅
- src/features/video/make-video/make-video-constants.ts: 페이지 내에서 사용하기 위한 상수가 별도로 필요하다면 여기 정의해줘 
- src/features/video/make-video/make-video-handlers.ts: 페이지 내에서 비즈니스로직을 구현하기 위해 함수 정의가 필요하다면, 여기 정의해줘
- src/features/video/make-video/make-video-model.ts: (구현완료) 페이지 내에서 비즈니스로직을 구현하기 위해 정의된 타입 (클린아키텍처 준수)    
- src/service/dto/video/make-video-dto.ts: (구현완료) 서비스 파일과 가까이 있으면서, 서비스파일에서 호출되는 타입(클린아키텍처 준수)  
- src/types/video/make-video-types.ts: (구현완료) -model과 -dto에서 공통으로 사용하는 타입 (클린아키텍처 준수)  
- src/service/template-service.ts: (구현완료) 해당 페이지에서 요청할 getRenderTemplateProductionList, getRenderTemplateProductionManualInfo, getRenderTemplateProductionImageList, createRenderTemplateProductionManual가 정의됨
