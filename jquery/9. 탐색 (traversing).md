...
// 5. find() : 자식요소 중 특정요소 찾기 (아래 두 코드는 같은 결과)
$('#root').find('li').addClass('s');
$('#root').addClass('s').removeClass('s').children().addClass('s').removeClass('s').first().addClass('s').removeClass('s').next().addClass('s').removeClass('s').prev().addClass('s').removeClass('s').parent().addClass('s').removeClass('s').find('li').addClass('s');
