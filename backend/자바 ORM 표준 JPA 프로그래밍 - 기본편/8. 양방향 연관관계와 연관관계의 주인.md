...
- 그래서 순수한 객체관계를 고려했 을 때, 연관관계 주인에도 값을 설정해주고, `mappedBy`를 사용하여 읽기전용 속성으로 관리되는 요소일지라도 순수한 객체 관계를 고려하여 업데이트를 해주어야 하는것이다.
	- 아니면 `em.flush(); em.clear();`를 중간에실행해주어야 한다.

- 그것도 아니면, Member class 엔티티 정의 ㅎ함수를 재정의해주면 된다. (연관관계 편의 메소)
```java
//...
@ManyToOne
private Team team;
