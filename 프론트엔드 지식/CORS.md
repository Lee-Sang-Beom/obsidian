...
- 자바스크립트의 fetch API를 사용하거나 Axios, Ajax 등을 사용할 때 서버로 쿠키를 함께 전송해야 하는 경우가 있을 수 있다.
	- 이 때, 인증과 관련된 정보를 담을 수 있게 해주는 **클라이언트 측**에서는 **withCredentials**과 같은 별도의 설정을 해 주어야 한다.
	- 별도 설정을 해주지 않으면  **인증정보는 자동으로 서버에 전송**되지 않는다.
