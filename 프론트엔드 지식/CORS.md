...
- CORS는 서로 다른 출처에서, 외부 리소스에 접근할 수 있는 권한을 부여할 수 있도록 브라우저에 알려주는 정책
- 브라우저는 **동일 출처 정책(SOP)** 을 지켜, 다른 **Origin**의 리소스 접근을 금지한다.
	- 따라서, 개발환경이나 Postman을 사용하여 API를 테스트할 때는 문제가 없다가, 브라우저 상에서 API를 호출할 때 문제가 발생하는 경우를 볼 수 있다.

- 여기서, **Origin**이란 출처를 의미하며, **Protocol + Host + Port** 를 합친 것을 말한다.
	- 당연히, Origin이 같으면 CORS 에러는 발생하지 않는다.
