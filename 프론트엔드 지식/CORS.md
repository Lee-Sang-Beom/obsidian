...
- 대체적인 순서는 위의 이미지와 같다고 생각하면 된다. 예시로 순서를 들어보자.
	...
	3. 서버가 예비 요청에 대한 응답으로, 서버가 어떤 요청을 수락할 수 있는지에 대한 정보를 담아 알려준다.
		- **Access-Control-Allow-Origin**: 요청이 허용된 출처(Origin)를 나타내는 헤더의 필드이다. 이 헤더를 통해 서버는 어떤 출처의 요청을 허용할지를 클라이언트에게 알려준다. 일반적으로 모든 출처`(*)` 혹은 실제 요청을 보낼 수 있는 출처가 여기에 포함된다.
		- **Access-Control-Allow-Methods**: 실제 요청에 허용되는 HTTP 메서드를 나타내는 헤더입니다. 서버가 어떤 HTTP 메서드를 허용하는지를 클라이언트에게 알려줍니다.
		- **Access-Control-Allow-Headers**: 실제 요청에 허용되는 HTTP 헤더를 나타내는 헤더입니다. 서버가 어떤 헤더를 허용하는지를 클라이언트에게 알려줍니다.
