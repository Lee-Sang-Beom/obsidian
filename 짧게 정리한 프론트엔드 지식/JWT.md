...
- 서버는 인증에 필요한 정보들을 암호화시켜 토큰을 발행하고, 클라이언트는 발행받은 토큰을 HTTP 헤더에 계속 넣어서 요청을 보내는 방식을 사용한다.
...
> - 토큰 정보를 표현하기 위하여 이미 정해진 종류의 기본 데이터들이다. (사용 권장됨)
> 	- iss: 토큰 발급자(issuer)
...
> 	- iat: 토큰 발급시간(issued at)으로, 토큰 발급 이후의 경과 시간
...
> [!note] **주의**
> - JWT는 Base64로 인코딩을 진행하기 때문에 디버거를 사용해서 인코딩된 JWT를 1초만에 복호화할 수 있다.
...
- Header와 Payload는 단순히 Base64로 인코딩되어 있을 뿐이라 누구나 쉽게 복호화가 가능하지만,  Signature는 서버 측에서 관리하는 비밀키가 유출되지 않는 이상 복호화할 수 없다
...
> 	- 만일 액세스 토큰의 시간이 만료되면 클라이언트는 리프레시 토큰을 이용해서, 서버로부터 새로운 엑세스 토큰을 발급받은 후 요청을 진행해야 정상적인 응답을 받을 수 있다.
> 
