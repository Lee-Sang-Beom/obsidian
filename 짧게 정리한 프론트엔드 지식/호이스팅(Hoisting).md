...
- 자바스크립트에서 소스 코드를 실행하는 과정은 **생성 단계**, **실행 단계**로 구분할 수 있다.
	...
	- **실행 단계 (Execution Phase)**: 코드가 실제로 실행되며, 변수에 값을 할당하고 함수가 호출된다. 그리고 이 때,실행 컨텍스트가 생성된다.
		- 실행 컨텍스트는 코드의 실행에 필요한 정보를 저장하는 객체로, 함수 호출 시마다 생성된다.
		- 이 컨텍스트는 **콜 스택(Call Stack)** 에 쌓이고, 함수가 종료되면 스택에서 제거된다.

- **자바스크립트 엔진**은 코드를 실행하기 전에 함수 안을 훑으면서, 실행 가능한 코드를 형상화하고 구분하는 과정을 거친다. 이것은 실행 컨텍스트를 위한 과정이다.
	- 이 실행 컨텍스트를 위한 과정에서 발생하는 것이 **호이스팅**이다. 
	- 즉, 위에서 언급한 **생성 단계**에서 코드의 일관성 있는 동작을 위해 스코프 내에 **선언된 식별자들에 대한 정보**를 기억하기 위한 과정으로, 아래쪽에 존재하는 내용 중 필요한 변수와 함수의 선언을 스코프 범위의 최상단으로 **내부적으로 끌어올려 처리**하는 것이 바로 **호이스팅**이라고 볼 수 있다는 것이다.
...
- 변수 선언(`var`)이 코드의 맨 위로 끌어올려지지만, 변수 선언 전에 해당 변수를 사용하면 `undefined` 값이 할당된다.
...
- 즉, **함수의 선언 자체가 코드의 최상단으로 끌어올려진다는 뜻**이다. 
	- 이러한 함수 선언문 방식은 선언과 동시에 함수가 생성된다는 특징을 가지기 때문에 **에러없이 정상 실행**된다.
	- 함수의 경우, 
```javascript
...
- `example` 함수가 호출되면서 자바스크립트 엔진은 새로운 **실행 컨텍스트**를 생성할 준비를 한다. 
	- 이 단계에서 변수와 함수 선언이 메모리에 등록(호이스팅)되지만, 실행 컨텍스트 자체는 아직 생성되지 않는다.
...
- 자바스크립트 엔진에 의해, 코드가 작성된 순서대로 실행된다.
...
- **호이스팅**을 통해 변수와 함수 선언이 미리 메모리로 올라가므로, 코드 실행 중에 이러한 선언들이 제대로 준비된 상태에서 실행이 시작된다.
	- 자바스크립트 생성단계에서 호이스팅이 발생하여 선언들이 모두 준비된 상태가 되고, 실행단계에서 실행 컨텍스트가 생기고 사라지면서 코드가 일관성있게 실행되니 맞는 말이다.
	- 예를 들어, 함수 호출이나 변수 참조 시 발생할 수 있는 **참조 오류**나 **타입 오류**를 미리 방지할 수 있다.
    
- **실행 컨텍스트**는 **콜 스택**에 쌓이면서, 각 함수의 실행에 필요한 정보를 관리한다.
	- 함수가 실행될 때마다 새로운 실행 컨텍스트가 생성되고, 함수가 끝나면 그 컨텍스트는 제거된다.
	- 이렇게 함으로써 **각 함수 호출에 독립적인 실행 환경을 제공**하고, 실행 흐름을 **안정적으로 제어**할 수 있게 된다.
