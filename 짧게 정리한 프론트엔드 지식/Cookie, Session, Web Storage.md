...
- 쿠키는 클라이언트(브라우저)에  **key-value** 형태로 저장되는 작은 파일이다.
	...
	- 사용자 인증이 유효한 시간을 명시할 수 있다.
		- 유효 시간이 설정된 경우, 브라우저 종료와 관계없이 쿠키가 유지된다.
		- 반대로 **유효 시간 설정이 없는 경우**는 세션 쿠키가 되어, 브라우저가 종료되면 쿠키도 삭제된다.
...
- **쿠키의 동작 방식**
	...
	3. 클라이언트는 서버로부터 받은 쿠키를 **로컬 PC**에 저장하고, 이후 서버에 요청할 때 해당 쿠키를 **HTTP 요청 헤더**의 `Cookie` 필드에 포함하여 전송한다.
	    - 동일 사이트 재방문과 같은 **동일한 요청**을 할 때, 클라이언트의 PC에 해당 쿠키가 있다면, 요청 페이지와 함께 **HTTP 요청 헤더**에 쿠키를 포함시켜 서버에 전송한다.
...
- **세션의 동작 방식**
	...
	3. 만약 **세션 ID**가 존재하지 않으면, 서버는 **세션 ID**를 생성해 클라이언트에 함께 돌려준다.
	...
	6. 서버는 **세션 ID**를 전달 받고, 세션 저장소에서 해당 **세션 ID**값을 찾아 클라이언트 정보를 가져와서 클라이언트를 구분한다. 그리고 각 클라이언트의 요구에 맞는 서비스를 제공한다.
