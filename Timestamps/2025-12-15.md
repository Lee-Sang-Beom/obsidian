...
- [x] 검수 목록 디렉터리 분류 및 일부 파일의 리팩토링 진행 작업
	- (리팩터링 예정) `src/pages/approve/modals` 디렉터리 내: `settings-approve-modal`, `settings-hold-modal`, `settings-modal` 분류

	- (리팩터링 예정) `src/pages/approve/sections/approve-settings-approve` 디렉터리 내: 승인/취소를 관리하는 승인 Form이 관리됨
	- (리팩터링 예정) `src/pages/approve/sections/approve-settings-hold` 디렉터리 내: 보류상태를 관리하는 Form이 관리됨 
	- (리팩터링 완료) `src/pages/approve/sections/approve-list` 디렉터리 내: `approve-search-section`, `approve-status-filter-section`, `columns-approve` 생성 및 분류 (테이블 리스트 표현을 위한 도메인 성격의 파일 분류: 리팩터링 완료)
	- (리팩터링 완료) `src/pages/approve/sections/approve-settings` 디렉터리 내: 기존의 `settings-form` 내부에서 관리되는 컴포넌트 분류 (리팩터링 완료)
		- `settins-form-ring`: 검수 팝업 내 설정타입 별 데이터를 표시하기 위한 파일로, 한 곳에 몰려있는 복잡성이 너무 깊은 코드에 대한 리팩토링 진행
			- `src/pages/approve/sections/approve-settings/campaign-ring`: container/presenter로 코드분리 진행. 캠페인 탭 설정정보 표시를 위한 파일 관리
			- `src/pages/approve/sections/approve-settings/default-ring`: container/presenter로 코드분리 진행. 기본음 탭 설정정보 표시를 위한 파일 관리
			- `src/pages/approve/sections/approve-settings/specific-date-ring`: container/presenter로 코드분리 진행. 특정일 탭 설정정보 표시를 위한 파일 관리
			- `src/pages/approve/sections/approve-settings/weekly-ring`: container/presenter로 코드분리 진행. 요일/시간대별 탭 설정정보 표시를 위한 파일 관리