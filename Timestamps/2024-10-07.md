...
---
- [x] (3:00) 코드하우스 교육
- [x] (1:00) 조사통계 - 클라이언트 csv 다운로드 구현
- [x] (2:00) 조사통계 - 모바일 구현 마무리 (Table 영역부터 하위 영역까지)
- [x] (2:00) 지도로보는김해 - 클러스터링 조사 및 성능확인
...
useEffect(() => {
    ...
    const handleMarkers = () => {
      ...
      let visibleMarkers = markerQuery.data.filter((marker) => {
        const position = new naver.maps.LatLng(marker.lat, marker.lot);
    ...
    const handleMarkersDebounced = debounce(handleMarkers, 200); // Adjust delay as needed
    handleMarkersDebounced();
    /**
     * @name cleanUp
