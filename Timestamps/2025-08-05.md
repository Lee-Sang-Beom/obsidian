#### Work
---
- [x] 페이지별 logScreenVIew 이벤트 기록 classViewName 개선
- [x] 팟캐스트 페이지 내 logScreenVIew tab별 이벤트 기록 추가
- [x] 구글 애널리틱스 팝업 오픈/닫기 등을 전용관리하는 이벤트 정보 메소드가 있는지 조사 
- [x] update 시간 변경 (1분 -> 10분), 팟캐스트 이미지 오버레이 조건식 변경, stockchart 컴포넌트 반투명로직 추가검사 시도
- [x] stock-chart의 커스텀 툴팁영역 별도 컴포넌트로 분리, 최고가 및 최저가의 텍스트를 최고 상승/하락률로 변경 및 국내매도에서는 최고하락률 미표시하도록 변경
- [x] refactor (features/refactor): 매수/매도 | 단기/중기/장기에 대한 사전 한글변환식 제거. signal constant.ts를 이용한 스타일 처리 유틸을 이용한 클래스명 부여
