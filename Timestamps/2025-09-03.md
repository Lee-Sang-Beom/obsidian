...
- [x] 문제해결필요: 베이직 1개월 -> 베이직 6개월 | 프리미엄 1개월 -> 프리미엄 6개월 적용 시 유저정보 조회용 me API가 FREE | FREE_PLAN으로 날아오는 이유
- [x] shadcn-ui Dialog open 상태에서 toast 발생 시 toast 창을 조작 가능하도록 변경 (pointer-event)
- [x] API 중복호출 의심되는 로직 보완 
- [x] 결제관련코드 가독성 완화
- [x] 결제 총 3단계에 대한 로딩 컴포넌트와 텍스트변경 반영
- [x] membership-status-card nextbling과 endDt 조합으로 멤버십 기간 표시하도록 변경
