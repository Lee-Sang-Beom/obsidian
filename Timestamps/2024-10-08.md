...
#### Work
---
- [x] (2:00) 데이터출처 페이징리스트조회 API 동작 오류 확인 / 원인분석 / 수정
- [x] (1:30) 지도로보는김해 CSS 수정
- [x] (2:00) 지도로보는김해 바운더리 마커조회 제거 및 관련 SIDE EFFECT 정리
- [x] (2:30) 데이터 출처 프론트엔드 퍼블리싱 및 일부 구현


#### 지도로 보는 김해 Client Code 백업
---
```tsx
"use client";
...
export default function GimhaeShowMapClient({ admdstGeomAllList }: IProps) {
  const htmlMarker1 = {
    content:
      '<div style="cursor:pointer;width:40px;height:40px;line-height:42px;font-size:10px;color:white;background-color:black;text-align:center;font-weight:bold;"></div>',
  };
  const htmlMarker2 = {
    content:
      '<div style="cursor:pointer;width:40px;height:40px;line-height:42px;font-size:10px;color:white;background-color:black;text-align:center;font-weight:bold;"></div>',
   ...
   *
   * @name gimhaePolygonGeomInstancesRef
   * @description polygon을 zoomLevel에 따라 동적으로 추가/제거하기위해서 필요한 ref로 일단 백업처리 중
   **/
  ...
  /**
   * @name useEffect
   * @description 인구/가구를 제외한 메뉴 변경 시 마다 페이징 초기화
   */
  useEffect(() => {
   ...
   *
   * - 주석한 useEffect 백업내용 : 폴리곤 확대했을 때 폴리곤 없어졌다가 축소하면 나오는거
   */
  // useEffect(() => {
  //   if (
  //     !isMounted ||
  ...
  /**
   * @name useEffect
   * @description 인구/가구 외 메뉴 변경 (마커변경)
   */
  useEffect(() => {
    ...
    if (markerQuery.data && markerQuery.data.length > 200) {
      ...
      markerQuery.data &&
      markerQuery.data.length > 0 &&
      markerQuery.data.length <= 200
    ) {
      // 중심이동 및 zoom 설정
      naverMapRef.current.setZoom(DEFAULT_MAP_ZOOM_LEVEL, false);
    } else {
    }
    ...
    const handleMarkers = () => {
      ...
      let visibleMarkers: MapGhMarkerResponse[] = [];
      ...
      const newNaverMarkerInstances: any[] = visibleMarkers.map(
        (marker: MapGhMarkerResponse, idx: number) => {
      ...
      // if (zoomLevel < boundaryZoomLevel && markerQuery.data.length > 200) {
      //   // 클러스터링 추가해야하면 이 코드 사용 (if문)
      //   let markerClustering = new MarkerClustering({
      //     minClusterSize: 2, // 최소 클러스터 마커 수
      //     maxZoom: boundaryZoomLevel, // 클러스터링이 해제되는 줌 레벨
      //     map: naverMapRef.current, // 클러스터링을 적용할 지도
      //     markers: newNaverMarkerInstances, // 마커 리스트
      //     disableClickZoom: false, // 클러스터 클릭 시 줌
      //     gridSize: 120, // 클러스터링을 할 그리드 크기
      //     icons: [
      //       htmlMarker1,
      //       htmlMarker2,
      //       htmlMarker3,
      ...
      //       htmlMarker5,
      //     ], // 클러스터 아이콘
      //     indexGenerator: [10, 100, 200, 500, 1000], // 클러스터링의 크기를 결정하는 기준
  ...
  /**
   * @name useEffect
   * @description인구/가구를 제외한 메뉴 변경 시 마다 테이블 초기화
   */
  useEffect(() => {
